...
void handle_stats(const httplib::Request&, httplib::Response& res, api_context& ctx) {
    write_json(res, ctx.stats.get_stats());
}

void handle_env(const httplib::Request&, httplib::Response& res, api_context&) {
...
void register_stats_handlers(route_registry& r) {
    r.add({http_method::get, "/api/stats", auth_policy::required, &handle_stats});
    r.add({http_method::get, "/api/env",   auth_policy::required, &handle_env});
}
