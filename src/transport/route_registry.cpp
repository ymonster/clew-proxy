...
// method, path, final status, and duration. Any error path (known
...
route_registry::route_registry(httplib::Server& server, api_context& ctx)
...
void route_registry::add(const route_def& def) {
    ...
    // Capture a reference to the external api_context (owned by main.cpp)
    // instead of capturing `this`. route_registry is usually a short-lived
