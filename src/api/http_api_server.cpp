// Implementation side of the hpp/cpp split for http_api_server.
// All non-template members live here; strand_sync<> stays inline in the
// header because a template method can't be explicitly instantiated
// without the caller's closure type.

#include "api/http_api_server.hpp"

// Shell / dialog APIs used only by /api/shell/* handlers
#include <shellapi.h>
#include <commdlg.h>

// Implementation-only standard headers
#include <algorithm>
#include <chrono>
#include <optional>
...
// ================================================================
// Construction / setters / lifecycle
// ================================================================

http_api_server::http_api_server(int port,
...
void http_api_server::set_static_dir(std::string_view dir) { static_dir_ = dir; }
void http_api_server::set_port_tracker(PortTracker* pt) { port_tracker_ = pt; }
void http_api_server::set_udp_port_tracker(UdpPortTracker* pt) { udp_port_tracker_ = pt; }
void http_api_server::set_icon_cache(icon_cache* ic) { icon_cache_ = ic; }
void http_api_server::set_on_config_change(std::function<void(const ConfigV2&)> cb) {
    on_config_change_ = std::move(cb);
}
...
int http_api_server::get_port() const { return port_; }
...
std::string http_api_server::hijack_source_from_match(const std::optional<RuleMatchResult>& match) {
    if (!match) return "";
...
std::pair<std::string, std::string> http_api_server::query_process_detail(DWORD pid) {
    // cmdline is always attempted (separate OpenProcess handle in query_process_cmdline)
...
json http_api_server::entry_to_api_json(const flat_tree& tree, const rule_engine_v3& rules,
...
std::string http_api_server::build_tree_snapshot() {
...
void http_api_server::publish_tree_snapshot() {
...
// ================================================================
// SSE broadcast + tree-changed entry point
// ================================================================

void http_api_server::broadcast_event(const std::string& event, const json& data) {
    std::scoped_lock lock(sse_mutex_);
    std::string message = "event: " + event + "\ndata: " + data.dump() + "\n\n";
    for (const auto* sink : sse_clients_) {
        if (sink->is_writable()) {
...
void http_api_server::broadcast_process_exit(DWORD pid) {
    json j;
    j["pid"] = pid;
    j["hijacked"] = false;
    j["exited"] = true;
...
void http_api_server::on_tree_changed() {
...
void http_api_server::setup_routes() {
    // --- CORS ---
    ...
    server_.Delete(R"(/api/proxy-groups/(\d+))", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        try {
            ...
            PC_LOG_WARN("[API] strand_sync threw during manual_count query: {}", e.what());
        }
