...
- **Command-line matching**: for cases like the same `python.exe` running different scripts, you can match by command-line keywords on the `Rules` page (e.g. proxy only the process running `crawler.py`, leave other python processes alone).
  - For example, [ccusage](https://github.com/ryoppippi/ccusage) fails with `Failed to fetch model pricing from LiteLLM` when its outbound request can't reach LiteLLM. On Node 24+ you can set `NODE_USE_ENV_PROXY=1` to make the HTTP proxy env vars effective; or, in Clew, add a rule on the Rules page with Name `ccusage`, Process name `node.exe`, and **Cmdline pattern `ccusage`** — that fixes it.
